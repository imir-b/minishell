...
t_ast	*ft_create_tree(t_token *first)
{
	t_token	*op;

	op = ft_find_operator(first, TOK_AND, TOK_OR);
	if (op)
		return (ft_create_and_or_node(op, first));
	op = ft_find_operator(first, TOK_PIPE, TOK_PIPE);
	if (op)
		return (ft_create_pipe_node(op, first));
	op = ft_find_operator(first, TOK_L_PAREN, TOK_L_PAREN);
	if (op)
		return (ft_create_subshell_node(op, first));
	op = ft_find_operator(first, TOK_REDIR_IN, TOK_HEREDOC);
	if (op)
		return (ft_create_redir_node(op, first));
	return (ft_create_command_node(first));
}
